...
async def process_agentic_pipeline(meeting_id: str, text: str):
    ...
    try:
        ...
        await broadcast(
            meeting_id,
            {"type": "processing_started", "meeting_id": meeting_id, "stages": stages},
        )

        for idx, stage in enumerate(stages[:4]):
            await broadcast(
                meeting_id,
                {"type": "analysis_stage", "stage_index": idx, "stage_text": stage},
            )
            await asyncio.sleep(0.2)

        # Perform AI extraction
        summary, items, decisions, risks_blockers, topics = (
            await extractor.extract_meeting_intelligence(meeting_id, text)
        )

        # Save summary
        await run_in_threadpool(database.save_meeting_summary, summary)
        await broadcast(
            meeting_id, {"type": "summary_extracted", "data": summary.dict()}
        )
        ...
        for d in decisions:
            ...
            await broadcast(
                meeting_id, {"type": "decision_extracted", "data": d.dict()}
            )
        ...
        for rb in risks_blockers:
            ...
            await broadcast(meeting_id, {"type": "risk_extracted", "data": rb.dict()})
            await asyncio.sleep(0.15)

        # Save Action Items line by line
        for item in items:
            await run_in_threadpool(database.save_action_item, item)
            await broadcast(meeting_id, {"type": "item_extracted", "data": item.dict()})
            await asyncio.sleep(0.25)

        # Save Topics line by line
        for t in topics:
            await run_in_threadpool(database.save_topic, t)
            await broadcast(meeting_id, {"type": "topic_extracted", "data": t.dict()})
            await asyncio.sleep(0.15)
...
async def process_uploaded_meeting(meeting_id: str, file_path: str, metadata: Dict):
    ...
    async def stage_update(stage: str, progress: float = 0.0):
        await broadcast(
            meeting_id, {"type": "stage_update", "stage": stage, "progress": progress}
        )
    ...
    cur.execute(
        ...
        (
            ...
            int(
                ...
                - datetime.fromisoformat(meeting.created_at).timestamp()
            ),
            meeting_id,
        ),
    )
...
async def update_item(meeting_id: str, item_id: str, payload: ActionItemUpdate):
    ...
    await broadcast(meeting_id, {"type": "item_updated", "data": item.dict()})
    return item
...
async def mark_item_complete(meeting_id: str, item_id: str):
    ...
    await broadcast(
        meeting_id,
        {"type": "item_completed", "data": {"id": item_id, "meeting_id": meeting_id}},
    )
    await broadcast(meeting_id, {"type": "agent_event", "data": event.dict()})
...
async def upload_meeting(
    ...
    if background_tasks is not None:
        background_tasks.add_task(
            process_uploaded_meeting, meeting_id, saved_path, metadata
        )
    else:
...
async def reset_meeting(meeting_id: str):
    ...
    await broadcast(meeting_id, {"type": "meeting_reset", "meeting_id": meeting_id})
    ...
    try:
        ...
        for m in meetings:
            ...
            if topics:
                topics_str = "\nExtracted Topics:\n" + "\n".join(
                    [
                        f"- {t.topic_name} ({t.start_time}-{t.end_time}): {t.summary}. Keywords: {', '.join(t.keywords)}"
                        for t in topics
                    ]
                )
            context += (
        ...
        print("Chat Error:", e)
        return {
            "answer": f"I received your question: '{payload.query}'. However, I was unable to process it with the AI provider. Error: {str(e)}"
        }
...
async def calendar_auth(provider: str):
    ...
    fake_auth_url = (
        f"/api/calendar/callback?provider={provider}&code=fake_auth_code_123"
    )
...
def get_calendar_accounts():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT provider FROM calendar_accounts WHERE user_id = 'default_user'"
    )
    ...
    **Participants:** (list based on speakers)
    ...
    try:
        ...
        minute = MeetingMinute(
            meeting_id=meeting_id, content=response, format="markdown"
        )
...
def export_meeting_minutes(meeting_id: str, format: str):
    ...
    return FileResponse(
        ...
        filename=f"Meeting_Minutes_{meeting_id}.{format}",
    )
