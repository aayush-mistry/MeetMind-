...
def start_agent_loop(meeting_id: str, broadcast_func: Callable[[str, dict], Awaitable[None]]):
...
async def _agent_loop(meeting_id: str, broadcast_func: Callable[[str, dict], Awaitable[None]]):
    """
    ...
    try:
        while True:
            await asyncio.sleep(20)  # 20 seconds compressed evaluation cycle for hackathon demo
            
            items = database.get_action_items(meeting_id)
            if not items:
                print(f"[Agent Loop] No items found for meeting '{meeting_id}'. Exiting cycle.")
                break

            all_completed = all(i.status == "completed" for i in items)
            if all_completed:
                print(f"[Agent Loop] All items resolved for meeting '{meeting_id}'. Stopping loop.")
                break
            ...
            for item in items:
                ...
                for dep in item.dependencies:
                    if dep.lower() in overdue_owners or any(i.owner.lower() == dep.lower() and i.status != "completed" for i in items):
                ...
                if is_dependency_at_risk and item.status != "at_risk":
                    ...
                    event = AgentEvent(
                        ...
                        signals=[f"Prerequisite owner {dep} has pending/overdue work" for dep in item.dependencies],
                        target=item.task_title
                    )
                ...
                if item.reminders_sent >= 3 and not item.escalated:
                    item.escalated = True
                    item.escalated_to = "Alex (Product Lead)"
                    item.status = "at_risk"
                    database.save_action_item(item)
                    ...
                    event = AgentEvent(
                        ...
                        type="item_escalated",
                        action="ESCALATE",
                        reason=f"3 automated reminders went unanswered by @{item.owner}. Escalating task to {item.escalated_to}.",
                        signals=["3 ignored reminders", "Deadline critical", "No completion response"],
                        target=f"@{item.owner} → {item.escalated_to}"
                    )
                    database.save_agent_event(event)
                ...
                if item.reminders_sent < 3:
                    ...
                    item.last_reminder_at = datetime.utcnow().isoformat() + "Z"
                    database.save_action_item(item)

                    # Trigger optional real email / simulation dispatch
                    email_status = _dispatch_email_reminder(item)
                    ...
                    event = AgentEvent(
                        ...
                        reason=f"Deadline approaching for '{item.task_title}' owned by @{item.owner}.",
                        signals=[
                            f"Task status: {item.status}",
                            f"Attempt count: #{item.reminders_sent}",
                    ...
                    await asyncio.sleep(1) # stagger between broadcast events
...
def _dispatch_email_reminder(item: ActionItem) -> str:
    ...
    if api_key:
        try:
            ...
            print(f"[Email Dispatcher] Sending real email to {item.owner}@company.com for task '{item.task_title}'")
