...
def init_db():
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_risks_blockers_meeting_id ON risks_blockers(meeting_id)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_agent_events_meeting_id ON agent_events(meeting_id)"
    )
    cursor.execute(
...
def get_topics(meeting_id: str) -> List[MeetingTopic]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM meeting_topics WHERE meeting_id = ? ORDER BY created_at ASC",
        (meeting_id,),
    )
    rows = cursor.fetchall()
    conn.close()
    ...
    for r in rows:
        ...
        topics.append(
            MeetingTopic(
                ...
                transcript_range=r["transcript_range"],
                created_at=r["created_at"],
            )
        )
    return topics
...
def get_action_item_by_id(meeting_id: str, item_id: str) -> Optional[ActionItem]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM action_items WHERE meeting_id = ? AND id = ?",
        (meeting_id, item_id),
    )
    r = cursor.fetchone()
    conn.close()
...
def get_risks_blockers(meeting_id: str) -> List[RiskBlocker]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM risks_blockers WHERE meeting_id = ? ORDER BY created_at ASC",
        (meeting_id,),
    )
    rows = cursor.fetchall()
    conn.close()
...
def get_agent_events(meeting_id: str) -> List[AgentEvent]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM agent_events WHERE meeting_id = ? ORDER BY timestamp DESC",
        (meeting_id,),
    )
    rows = cursor.fetchall()
    conn.close()
...
def delete_calendar_account(user_id: str, provider: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE FROM calendar_accounts WHERE user_id = ? AND provider = ?",
        (user_id, provider),
    )
    conn.commit()
    conn.close()
...
def link_calendar_event(event_id: str, meeting_id: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE calendar_events SET meeting_id = ? WHERE event_id = ?",
        (meeting_id, event_id),
    )
    conn.commit()
    conn.close()
